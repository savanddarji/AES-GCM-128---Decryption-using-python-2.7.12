...
#    print 'Pdash:',hexad(''.join(Pdash[len(Pdash)-1]))
    ...
    while j<len(Cdash):
        ...
        print 'E( K,Y',i,'):',dash
        dash=part(dash)
        temp=xor(dash[0],Cdash[j])
        P1.append(hexad(temp))
        j+=1
        i+=1
#    print C1
    if len(C)%32==0:
        ...
        scall=len(P1)-1
        Ptemp=P1[scall]
        Ptemp=Ptemp[0:len(C[(32*(len(C)/32)):])]
        del P1[scall]
        P1.insert(scall,Ptemp)
    P1=''.join(P1)
#    print C1
X1=GHASH(H1,A1,C)
Tag=xor(X1,Ysave[0])
...
print 'P:',P1
print 'T:',Tag
print 'Authorisation:',acheck(Tag,Tag1)
